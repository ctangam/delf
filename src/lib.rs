...
impl File {
    ...
    pub fn parse(i: parse::Input) -> parse::Result<Self> {
        ...
        let (i, (sh_entsize, sh_count, sh_nidx)) = tuple((u16_usize, le_u16, le_u16))(i)?;

        let ph_slices = (&full_input[ph_offset.into()..]).chunks(ph_entsize as usize);
        let mut program_headers = Vec::new();
        for ph_slice in ph_slices.take(ph_count as usize) {
            let (_, ph) = ProgramHeader::parse(full_input, ph_slice)?;
            program_headers.push(ph);
        }

        let sh_slices = (&full_input[sh_offset.into()..]).chunks(sh_entsize as usize);
        let mut section_headers = Vec::new();
        for sh_slice in sh_slices.take(sh_count as usize) {
